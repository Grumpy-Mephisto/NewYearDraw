...
public class Firework {
    ...
    public void draw(Graphics2D g2d, int width, int height, int numFireworks,
            int numLinesPerFirework) {
        for (int i = 0; i < numFireworks; i++) {
            ...
            for (int j = 0; j < numLinesPerFirework; j++) {
                ...
                int endXAdjusted = (int) (startX
                        + length * Math.cos(Math.atan2(endY - startY, endX - startX)));
                int endYAdjusted = (int) (startY
                        + length * Math.sin(Math.atan2(endY - startY, endX - startX)));
